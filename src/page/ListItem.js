...
class ListItem extends Component {
    ...
    render() {
        ...
        return (<div>
            ...
            <span style={{ display: !this.state.edit ? 'inline-block' : 'none' }} onClick={() => { this.editItem(pdata.checked)}}  className={pdata.checked ? 'text finish' : 'text'}>{pdata.text}</span>
            <input 
            onBlur={(e) => {this.editItemText(e)}} 
            onKeyDown={(e) => { this.handleKeyEnter(e)}} 
            ref={this.inputRef} 
            onChange={(e) => { this.handleInputChange(e) }}
            style={{display: this.state.edit? 'inline-block' : 'none'}} 
            type="text" value={this.state.text}></input>
